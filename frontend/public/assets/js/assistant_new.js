...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleSendMessage() {
        ...
        try {
            ...
            if (!assistant.currentConversationId) {
                ...
                assistant.currentConversationId = data._id;
            }

            // Add user's question to chat and save to database
            addMessage(question, true);
            const messageResponse = await fetch(`/api/conversations/${assistant.currentConversationId}/messages`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
            ...
            const aiResponse = await assistant.processQuestion(question);
            addMessage(aiResponse.message);

            // Save AI response to database
            const aiMessageResponse = await fetch(`/api/conversations/${assistant.currentConversationId}/messages`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    async function handleNewConversation() {
        try {
            ...
            if (chatBox.children.length > 1) {
                const confirm = window.confirm('Start a new conversation? This will clear the current chat.');
                if (!confirm) return;
            }
            ...
            successMessage.className = 'flex items-center justify-center py-2 px-4 bg-green-100 text-green-700 rounded-lg mb-4';
            ...
            `;
            chatBox.insertBefore(successMessage, chatBox.firstChild.nextSibling);
            
            // Remove success message after 3 seconds
            setTimeout(() => {
                if (successMessage.parentNode === chatBox) {
                    chatBox.removeChild(successMessage);
                }
            }, 3000);
        } catch (error) {
    ...
    async function handleDeleteConversation() {
        try {
            ...
            const response = await fetch(`/api/conversations/${assistant.currentConversationId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            ...
            assistant.currentConversationId = null;
            ...
            // Show delete confirmation with icon
            const deleteMessage = document.createElement('div');
            deleteMessage.className = 'flex items-center justify-center py-2 px-4 bg-red-100 text-red-700 rounded-lg mb-4';
            ...
            `;
            chatBox.insertBefore(deleteMessage, chatBox.firstChild.nextSibling);
            
            // Remove delete message after 3 seconds
            setTimeout(() => {
                if (deleteMessage.parentNode === chatBox) {
                    chatBox.removeChild(deleteMessage);
                }
            }, 3000);
        } catch (error) {
